...
- Chromium、Firefox、WebKit のbrowser test。各jobはNix環境でapplicationを事前buildし、project packageと同じversionかつdigest固定した公式Playwright image内でbrowserを起動する。最初の失敗で停止し、job全体は15分で打ち切る
